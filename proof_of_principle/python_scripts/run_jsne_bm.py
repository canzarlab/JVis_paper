...
data_name = "output/" + data_set #output
...
for noise_level in [0, 0.1, 0.2, 0.3, 0.4]:
    ...
    for _lambda in [0.5, 1, 2, 3]:
        joint_tsne_obj = JTSNE(init='random', n_iter = 5000, n_jobs=nJobs)
        data = {'modal-1': expr_reduced, 'modal-2': adt_mat,  'modal-noise': expr_reduced_shufle}
        joint_tsne = joint_tsne_obj.fit_transform(X = data, method = 'auto', _lambda = _lambda, max_iter=maxIter)
        # save data
        
        np.savetxt(data_name +"_OPTJsne_ld" + str(_lambda)+"_noise" + str(noise_level) +".csv", joint_tsne, delimiter=",")
        np.savetxt(data_name +"_alpha_lambda" + str(_lambda) +"_noise" + str(noise_level)+ ".csv", np.array(joint_tsne_obj.alpha), delimiter=",")

    # Run Jvis (uniform)
    print("Run uniform Jvis")
    joint_tsne_obj_uni = JTSNE(init='random', n_iter = 5000, n_jobs=nJobs)
    joint_tsne_uni = joint_tsne_obj_uni.fit_transform(X = data, method = 'uniform')
    # Save data
    np.savetxt(data_name +"_uniformJsne"+"_noise" + str(noise_level)+ ".csv", joint_tsne_uni, delimiter=",")
    ...
    concat_tsne = TSNE(init='random', n_iter = 5000, n_jobs=nJobs).fit_transform(expr_reduced)
    # save data
    np.savetxt(data_name +"_concatFroTsne"+"_noise" + str(noise_level)+ ".csv", concat_tsne, delimiter=",")
    print("Run concat")
    ...
    concat_tsne = TSNE(init='random', n_iter = 5000, n_jobs=nJobs).fit_transform(expr_reduced)
    # save data
    np.savetxt(data_name +"_concatMaxTsne"+"_noise" + str(noise_level)+ ".csv", concat_tsne, delimiter=",")
    print("Run concat")
    concat = np.concatenate((expr_mat_log_t, adt_mat, expr_mat_shuffle), axis = 1)
    expr_reduced = PCA(n_components=100).fit_transform(concat)
    concat_tsne = TSNE(init='random', n_iter = 5000, n_jobs=nJobs).fit_transform(expr_reduced)
    # save data
    np.savetxt(data_name +"_concatTsne"+"_noise" + str(noise_level)+ ".csv", concat_tsne, delimiter=",")
